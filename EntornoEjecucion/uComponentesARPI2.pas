...
begin
  ...
  for i := 0 to values.Count - 1 do
    if name<>'' then
      TEntornoDesarrollo.instance.setearPropiedad(name, 'caption' ,values.ValueFromIndex[i], values.Names[i]);
  inherited Caption := values.ValueFromIndex[0];
end;
...
begin
  ...
  if lowercase(aux) ='numero' then
    ...
    result := Letra
  else if lowercase(aux) ='texto' then
    result := Texto;
end;
...
var
  i: Integer;
begin
  inherited Items.Commatext := Value.ValueFromIndex[0]; //TODO: Idioma predeterminado//
...
begin
  for i := 0 to value.Count - 1 do
    if name<>'' then
      TEntornoDesarrollo.instance.setearPropiedad(name, 'items', value.ValueFromIndex[i], value.Names[i]);
  inherited Items.CommaText := value.ValueFromIndex[0]; //TODO: cambiar el primer idioma por el predeterminado//
end;
